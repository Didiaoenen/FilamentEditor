...
FEditor::Core::CameraController::CameraController
(
	FEditor::Panels::AView& p_view,
...
void FEditor::Core::CameraController::HandleInputs(float p_deltaTime)
{
	if (m_view.IsHovered())
	{
		UpdateMouseState();
		...
		if (!ImGui::IsAnyItemActive() && m_enableFocusInputs)
		{
			if (EDITOR_EXEC(IsAnyActorSelected()))
			{
				...
				if (m_inputManager.IsKeyPressed(FWindow::Inputs::EKey::KEY_F))
				{
				...
				if (m_inputManager.IsKeyPressed(FWindow::Inputs::EKey::KEY_UP))			focusObjectFromAngle(OvMaths::FVector3::Up);
				if (m_inputManager.IsKeyPressed(FWindow::Inputs::EKey::KEY_DOWN))		focusObjectFromAngle(-OvMaths::FVector3::Up);
				if (m_inputManager.IsKeyPressed(FWindow::Inputs::EKey::KEY_RIGHT))		focusObjectFromAngle(OvMaths::FVector3::Right);
				if (m_inputManager.IsKeyPressed(FWindow::Inputs::EKey::KEY_LEFT))		focusObjectFromAngle(-OvMaths::FVector3::Right);
				if (m_inputManager.IsKeyPressed(FWindow::Inputs::EKey::KEY_PAGE_UP))	focusObjectFromAngle(OvMaths::FVector3::Forward);
				if (m_inputManager.IsKeyPressed(FWindow::Inputs::EKey::KEY_PAGE_DOWN))	focusObjectFromAngle(-OvMaths::FVector3::Forward);
			}
		}
	}
	...
	else
	{
		if (m_rightMousePressed || m_middleMousePressed || m_leftMousePressed)
		{
			...
			else
			{
				...
				{
					if (m_inputManager.GetKeyState(FWindow::Inputs::EKey::KEY_LEFT_ALT) == FWindow::Inputs::EKeyState::KEY_DOWN)
					{
						if (EDITOR_EXEC(IsAnyActorSelected()))
						{
...
void FEditor::Core::CameraController::MoveToTarget(OvCore::ECS::Actor& p_target)
{
...
void FEditor::Core::CameraController::SetSpeed(float p_speed)
{
	m_cameraMoveSpeed = p_speed;
}

float FEditor::Core::CameraController::GetSpeed() const
{
	return m_cameraMoveSpeed;
}

void FEditor::Core::CameraController::SetPosition(const OvMaths::FVector3 & p_position)
{
	m_cameraPosition = p_position;
}

void FEditor::Core::CameraController::SetRotation(const OvMaths::FQuaternion & p_rotation)
{
	m_cameraRotation = p_rotation;
}

const OvMaths::FVector3& FEditor::Core::CameraController::GetPosition() const
{
	return m_cameraPosition;
}

const OvMaths::FQuaternion& FEditor::Core::CameraController::GetRotation() const
{
	return m_cameraRotation;
}

bool FEditor::Core::CameraController::IsRightMousePressed() const
{
	return m_rightMousePressed;
}

void FEditor::Core::CameraController::HandleCameraPanning(const OvMaths::FVector2& p_mouseOffset, bool p_firstMouset)
{
	m_window.SetCursorShape(FWindow::Cursor::ECursorShape::HAND);
...
void FEditor::Core::CameraController::HandleCameraOrbit(const OvMaths::FVector2& p_mouseOffset, bool p_firstMouse)
{
...
void FEditor::Core::CameraController::HandleCameraZoom()
{
	m_cameraPosition += m_cameraRotation * OvMaths::FVector3::Forward * ImGui::GetIO().MouseWheel;
}

void FEditor::Core::CameraController::HandleCameraFPSMouse(const OvMaths::FVector2& p_mouseOffset, bool p_firstMouse)
{
...
void FEditor::Core::CameraController::HandleCameraFPSKeyboard(float p_deltaTime)
{
	m_targetSpeed = OvMaths::FVector3(0.f, 0.f, 0.f);

	if (m_rightMousePressed)
	{
		bool run = m_inputManager.GetKeyState(FWindow::Inputs::EKey::KEY_LEFT_SHIFT) == FWindow::Inputs::EKeyState::KEY_DOWN;
		float velocity = m_cameraMoveSpeed * p_deltaTime * (run ? 2.0f : 1.0f);

		if (m_inputManager.GetKeyState(FWindow::Inputs::EKey::KEY_W) == FWindow::Inputs::EKeyState::KEY_DOWN)
			m_targetSpeed += m_cameraRotation * OvMaths::FVector3::Forward * velocity;
		if (m_inputManager.GetKeyState(FWindow::Inputs::EKey::KEY_S) == FWindow::Inputs::EKeyState::KEY_DOWN)
			m_targetSpeed += m_cameraRotation * OvMaths::FVector3::Forward * -velocity;
		if (m_inputManager.GetKeyState(FWindow::Inputs::EKey::KEY_A) == FWindow::Inputs::EKeyState::KEY_DOWN)
			m_targetSpeed += m_cameraRotation * OvMaths::FVector3::Right * velocity;
		if (m_inputManager.GetKeyState(FWindow::Inputs::EKey::KEY_D) == FWindow::Inputs::EKeyState::KEY_DOWN)
			m_targetSpeed += m_cameraRotation * OvMaths::FVector3::Right * -velocity;
		if (m_inputManager.GetKeyState(FWindow::Inputs::EKey::KEY_E) == FWindow::Inputs::EKeyState::KEY_DOWN)
			m_targetSpeed += {0.0f, velocity, 0.0f};
		if (m_inputManager.GetKeyState(FWindow::Inputs::EKey::KEY_Q) == FWindow::Inputs::EKeyState::KEY_DOWN)
			m_targetSpeed += {0.0f, -velocity, 0.0f};
	}
...
void FEditor::Core::CameraController::UpdateMouseState()
{
	if (m_inputManager.IsMouseButtonPressed(FWindow::Inputs::EMouseButton::MOUSE_BUTTON_LEFT))
		m_leftMousePressed = true;

	if (m_inputManager.IsMouseButtonReleased(FWindow::Inputs::EMouseButton::MOUSE_BUTTON_LEFT))
	{
		m_leftMousePressed = false;
		m_firstMouse = true;
	}

	if (m_inputManager.IsMouseButtonPressed(FWindow::Inputs::EMouseButton::MOUSE_BUTTON_MIDDLE))
		m_middleMousePressed = true;

	if (m_inputManager.IsMouseButtonReleased(FWindow::Inputs::EMouseButton::MOUSE_BUTTON_MIDDLE))
	{
		m_middleMousePressed = false;
		m_firstMouse = true;
	}

	if (m_inputManager.IsMouseButtonPressed(FWindow::Inputs::EMouseButton::MOUSE_BUTTON_RIGHT))
	{
		m_rightMousePressed = true;
		m_window.SetCursorMode(FWindow::Cursor::ECursorMode::DISABLED);
	}

	if (m_inputManager.IsMouseButtonReleased(FWindow::Inputs::EMouseButton::MOUSE_BUTTON_RIGHT))
	{
		m_rightMousePressed = false;
		m_firstMouse = true;
		m_window.SetCursorMode(FWindow::Cursor::ECursorMode::NORMAL);
	}
}
