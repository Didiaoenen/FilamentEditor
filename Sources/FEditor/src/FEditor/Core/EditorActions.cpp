...
FEditor::Core::EditorActions::EditorActions(Context& p_context, EditorRenderer& p_editorRenderer, PanelsManager& p_panelsManager) :
	m_context(p_context), 
	m_renderer(p_editorRenderer),
	m_panelsManager(p_panelsManager)
{
	OvCore::Global::ServiceLocator::Provide<FEditor::Core::EditorActions>(*this);
...
void FEditor::Core::EditorActions::LoadEmptyScene()
{
...
void FEditor::Core::EditorActions::SaveCurrentSceneTo(const std::string& p_path)
{
...
void FEditor::Core::EditorActions::LoadSceneFromDisk(const std::string& p_path, bool p_absolute)
{
	if (GetCurrentEditorMode() != EEditorMode::EDIT)
		StopPlaying();
	...
	m_panelsManager.GetPanelAs<FEditor::Panels::SceneView>("Scene View").Focus();
}

bool FEditor::Core::EditorActions::IsCurrentSceneLoadedFromDisk() const
{
	return m_context.sceneManager.IsCurrentSceneLoadedFromDisk();
}

void FEditor::Core::EditorActions::SaveSceneChanges()
{
...
void FEditor::Core::EditorActions::SaveAs()
{
	FWindow::Dialogs::SaveFileDialog dialog("New Scene");
	dialog.SetInitialDirectory(m_context.projectAssetsPath + "New Scene");
	dialog.DefineExtension("FilamentEditor Scene", ".ovscene");
	dialog.Show();
	...
	if (dialog.HasSucceeded())
	{
		...
		{
			FWindow::Dialogs::MessageBox message("File already exists!", "The file \"" + dialog.GetSelectedFileName() + "\" already exists.\n\nUsing this file as the new home for your scene will erase any content stored in this file.\n\nAre you ok with that?", FWindow::Dialogs::MessageBox::EMessageType::WARNING, FWindow::Dialogs::MessageBox::EButtonLayout::YES_NO, true);
			switch (message.GetUserAction())
			{
			case FWindow::Dialogs::MessageBox::EUserAction::YES: break;
			case FWindow::Dialogs::MessageBox::EUserAction::NO: return;
			}
		}
...
void FEditor::Core::EditorActions::RefreshScripts()
{
...
std::optional<std::string> FEditor::Core::EditorActions::SelectBuildFolder()
{
	FWindow::Dialogs::SaveFileDialog dialog("Build location");
	...
	if (dialog.HasSucceeded())
	{
		...
		{
			FWindow::Dialogs::MessageBox message("Folder already exists!", "The folder \"" + result + "\" already exists.\n\nPlease select another location and try again", FWindow::Dialogs::MessageBox::EMessageType::WARNING, FWindow::Dialogs::MessageBox::EButtonLayout::OK, true);
			return {};
		}
	}
	else
...
void FEditor::Core::EditorActions::Build(bool p_autoRun, bool p_tempFolder)
{
	std::string destinationFolder;

	if (p_tempFolder)
	{
		destinationFolder = std::string(getenv("APPDATA")) + "\\OverloadTech\\FEditor\\TempBuild\\";
		try
		{
		...
		{
			FWindow::Dialogs::MessageBox message("Temporary build failed", "The temporary folder is currently being used by another process", FWindow::Dialogs::MessageBox::EMessageType::ERROR, FWindow::Dialogs::MessageBox::EButtonLayout::OK, true);
			return;
		}
	}
...
void FEditor::Core::EditorActions::BuildAtLocation(const std::string & p_configuration, const std::string p_buildPath, bool p_autoRun)
{
			...
			if (std::filesystem::create_directory(buildPath + "Data\\User\\"))
			{
				...
				if (!err)
				{
					...
					if (!std::filesystem::exists(buildPath + "Data\\User\\Assets\\" + (m_context.projectSettings.Get<std::string>("start_scene"))))
					{
						...
						FWindow::Dialogs::MessageBox message("Build Failure", "An error occured during the building of your game.\nCheck the console for more information", FWindow::Dialogs::MessageBox::EMessageType::ERROR, FWindow::Dialogs::MessageBox::EButtonLayout::OK, true);
						std::filesystem::remove_all(buildPath);
						return;						
					}
						...
						if (!err)
						{
							...
							if (p_autoRun)
							{
								...
								if (std::filesystem::exists(exePath))
									FTools::Utils::SystemCalls::OpenFile(exePath, buildPath);
								else
								{
...
void FEditor::Core::EditorActions::DelayAction(std::function<void()> p_action, uint32_t p_frames)
{
	m_delayedActions.emplace_back(p_frames + 1, p_action);
}

void FEditor::Core::EditorActions::ExecuteDelayedActions()
{
...
FEditor::Core::Context& FEditor::Core::EditorActions::GetContext()
{
	return m_context;
}

FEditor::Core::EditorRenderer & FEditor::Core::EditorActions::GetRenderer()
{
	return m_renderer;
}

FEditor::Core::PanelsManager& FEditor::Core::EditorActions::GetPanelsManager()
{
	return m_panelsManager;
}

void FEditor::Core::EditorActions::SetActorSpawnAtOrigin(bool p_value)
{
	if (p_value)
...
void FEditor::Core::EditorActions::SetActorSpawnMode(EActorSpawnMode p_value)
{
	m_actorSpawnMode = p_value;
}

void FEditor::Core::EditorActions::ResetLayout()
{
    DelayAction([this]() {m_context.uiManager->ResetLayout("Config\\layout.ini"); });
}

void FEditor::Core::EditorActions::SetSceneViewCameraSpeed(int p_speed)
{
	EDITOR_PANEL(Panels::SceneView, "Scene View").GetCameraController().SetSpeed((float)p_speed);
}

int FEditor::Core::EditorActions::GetSceneViewCameraSpeed()
{
	return (int)EDITOR_PANEL(Panels::SceneView, "Scene View").GetCameraController().GetSpeed();
}

void FEditor::Core::EditorActions::SetAssetViewCameraSpeed(int p_speed)
{
	EDITOR_PANEL(Panels::AssetView, "Asset View").GetCameraController().SetSpeed((float)p_speed);
}

int FEditor::Core::EditorActions::GetAssetViewCameraSpeed()
{
	return (int)EDITOR_PANEL(Panels::AssetView, "Asset View").GetCameraController().GetSpeed();
}

void FEditor::Core::EditorActions::ResetSceneViewCameraPosition()
{
...
void FEditor::Core::EditorActions::ResetAssetViewCameraPosition()
{
...
FEditor::Core::EditorActions::EEditorMode FEditor::Core::EditorActions::GetCurrentEditorMode() const
{
	return m_editorMode;
}

void FEditor::Core::EditorActions::SetEditorMode(EEditorMode p_newEditorMode)
{
	m_editorMode = p_newEditorMode;
	EditorModeChangedEvent.Invoke(m_editorMode);
}

void FEditor::Core::EditorActions::StartPlaying()
{
	if (m_editorMode == EEditorMode::EDIT)
	{
		...
		if (m_context.scriptInterpreter->IsOk())
		{
			...
			m_panelsManager.GetPanelAs<FEditor::Panels::GameView>("Game View").Focus();
...
void FEditor::Core::EditorActions::PauseGame()
{
	SetEditorMode(EEditorMode::PAUSE);
	m_context.audioEngine->Suspend();
}

void FEditor::Core::EditorActions::StopPlaying()
{
	if (m_editorMode != EEditorMode::EDIT)
	{
		ImGui::GetIO().DisableMouseUpdate = false;
		m_context.window->SetCursorMode(FWindow::Cursor::ECursorMode::NORMAL);
...
void FEditor::Core::EditorActions::NextFrame()
{
	if (m_editorMode == EEditorMode::PLAY || m_editorMode == EEditorMode::PAUSE)
		SetEditorMode(EEditorMode::FRAME_BY_FRAME);
}

OvMaths::FVector3 FEditor::Core::EditorActions::CalculateActorSpawnPoint(float p_distanceToCamera)
{
...
OvCore::ECS::Actor & FEditor::Core::EditorActions::CreateEmptyActor(bool p_focusOnCreation, OvCore::ECS::Actor* p_parent, const std::string& p_name)
{
...
OvCore::ECS::Actor & FEditor::Core::EditorActions::CreateActorWithModel(const std::string& p_path, bool p_focusOnCreation, OvCore::ECS::Actor* p_parent, const std::string& p_name)
{
...
bool FEditor::Core::EditorActions::DestroyActor(OvCore::ECS::Actor & p_actor)
{
...
std::string FindDuplicatedActorUniqueName(OvCore::ECS::Actor& p_duplicated, OvCore::ECS::Actor& p_newActor, OvCore::SceneSystem::Scene& p_scene)
{
    ...
    return FTools::Utils::String::GenerateUnique(p_duplicated.GetName(), availabilityChecker);
}

void FEditor::Core::EditorActions::DuplicateActor(OvCore::ECS::Actor & p_toDuplicate, OvCore::ECS::Actor* p_forcedParent, bool p_focus)
{
	tinyxml2::XMLDocument doc;
...
void FEditor::Core::EditorActions::SelectActor(OvCore::ECS::Actor & p_target)
{
	EDITOR_PANEL(Panels::Inspector, "Inspector").FocusActor(p_target);
}

void FEditor::Core::EditorActions::UnselectActor()
{
	EDITOR_PANEL(Panels::Inspector, "Inspector").UnFocus();
}

bool FEditor::Core::EditorActions::IsAnyActorSelected() const
{
	return EDITOR_PANEL(Panels::Inspector, "Inspector").GetTargetActor();
}

OvCore::ECS::Actor & FEditor::Core::EditorActions::GetSelectedActor() const
{
	return *EDITOR_PANEL(Panels::Inspector, "Inspector").GetTargetActor();
}

void FEditor::Core::EditorActions::MoveToTarget(OvCore::ECS::Actor& p_target)
{
	EDITOR_PANEL(Panels::SceneView, "Scene View").GetCameraController().MoveToTarget(p_target);
}

void FEditor::Core::EditorActions::CompileShaders()
{
...
void FEditor::Core::EditorActions::SaveMaterials()
{
...
bool FEditor::Core::EditorActions::ImportAsset(const std::string& p_initialDestinationDirectory)
{
	...
	if (selectAssetDialog.HasSucceeded())
	{
		...
		std::string extension = '.' + FTools::Utils::PathParser::GetExtension(source);
...
bool FEditor::Core::EditorActions::ImportAssetAtLocation(const std::string& p_destination)
{
	using namespace FWindow::Dialogs;
...
std::string FEditor::Core::EditorActions::GetRealPath(const std::string & p_path)
{
	std::string result;
...
std::string FEditor::Core::EditorActions::GetResourcePath(const std::string & p_path, bool p_isFromEngine)
{
	std::string result = p_path;

	if (FTools::Utils::String::Replace(result, p_isFromEngine ? m_context.engineAssetsPath : m_context.projectAssetsPath, ""))
	{
		if (p_isFromEngine)
			result = ':' + result;
	}
...
std::string FEditor::Core::EditorActions::GetScriptPath(const std::string & p_path)
{
	std::string result = p_path;

	FTools::Utils::String::Replace(result, m_context.projectScriptsPath, "");
	FTools::Utils::String::Replace(result, ".lua", "");

	return result;
}

void FEditor::Core::EditorActions::PropagateFolderRename(std::string p_previousName, std::string p_newName)
{
	p_previousName = FTools::Utils::PathParser::MakeNonWindowsStyle(p_previousName);
	p_newName = FTools::Utils::PathParser::MakeNonWindowsStyle(p_newName);

	for (auto& p : std::filesystem::recursive_directory_iterator(p_newName))
	{
		if (!p.is_directory())
		{
			std::string newFileName = FTools::Utils::PathParser::MakeNonWindowsStyle(p.path().string());
			...
			PropagateFileRename(FTools::Utils::PathParser::MakeWindowsStyle(previousFileName), FTools::Utils::PathParser::MakeWindowsStyle(newFileName));
		}
	}
}

void FEditor::Core::EditorActions::PropagateFolderDestruction(std::string p_folderPath)
{
	for (auto& p : std::filesystem::recursive_directory_iterator(p_folderPath))
	{
		if (!p.is_directory())
		{
			PropagateFileRename(FTools::Utils::PathParser::MakeWindowsStyle(p.path().string()), "?");
		}
	}
}

void FEditor::Core::EditorActions::PropagateScriptRename(std::string p_previousName, std::string p_newName)
{
	...
	PropagateFileRenameThroughSavedFilesOfType(p_previousName, p_newName, FTools::Utils::PathParser::EFileType::SCENE);

	EDITOR_PANEL(Panels::Inspector, "Inspector").Refresh();
}

void FEditor::Core::EditorActions::PropagateFileRename(std::string p_previousName, std::string p_newName)
{
	...
	switch (FTools::Utils::PathParser::GetFileType(p_previousName))
	{
	...
	case FTools::Utils::PathParser::EFileType::MODEL:
		PropagateFileRenameThroughSavedFilesOfType(p_previousName, p_newName, FTools::Utils::PathParser::EFileType::SCENE);
		break;
	case FTools::Utils::PathParser::EFileType::SHADER:
		PropagateFileRenameThroughSavedFilesOfType(p_previousName, p_newName, FTools::Utils::PathParser::EFileType::MATERIAL);
		break;
	case FTools::Utils::PathParser::EFileType::TEXTURE:
		PropagateFileRenameThroughSavedFilesOfType(p_previousName, p_newName, FTools::Utils::PathParser::EFileType::MATERIAL);
		break;
	case FTools::Utils::PathParser::EFileType::SOUND:
		PropagateFileRenameThroughSavedFilesOfType(p_previousName, p_newName, FTools::Utils::PathParser::EFileType::SCENE);
		break;
	}
...
void FEditor::Core::EditorActions::PropagateFileRenameThroughSavedFilesOfType(const std::string& p_previousName, const std::string& p_newName, FTools::Utils::PathParser::EFileType p_fileType)
{
	for (auto& entry : std::filesystem::recursive_directory_iterator(m_context.projectAssetsPath))
	{
		if (FTools::Utils::PathParser::GetFileType(entry.path().string()) == p_fileType)
		{
			using namespace std;

			{
				...
				{
					if (FTools::Utils::String::Replace(line, wordToReplace, wordToReplaceWith))
