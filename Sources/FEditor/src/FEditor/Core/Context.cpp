...
FEditor::Core::Context::Context(const std::string& p_projectPath, const std::string& p_projectName) :
	...
	projectSettings(projectFilePath)
{
	...
	windowSettings.title = "FilamentEditor Editor";
	windowSettings.width = 1600;
	windowSettings.height = 900;

	/* Window creation */
	device = std::make_unique<FWindow::Context::Device>(deviceSettings);
	window = std::make_unique<FWindow::Window>(*device, windowSettings);
	...
	inputManager = std::make_unique<FWindow::Inputs::InputManager>(*window);
	...
	uiManager = std::make_unique<FEditorUI::Core::UIManager>(window->GetGlfwWindow(), FEditorUI::Styling::EStyle::ALTERNATIVE_DARK);
	...
	uiManager->SetEditorLayoutSaveFilename(std::string(getenv("APPDATA")) + "\\OverloadTech\\FEditor\\layout.ini");
	uiManager->SetEditorLayoutAutosaveFrequency(60.0f);
	uiManager->EnableEditorLayoutSave(true);
	uiManager->EnableDocking(true);

	if (!std::filesystem::exists(std::string(getenv("APPDATA")) + "\\OverloadTech\\FEditor\\layout.ini"))
	...
	/* Editor resources */
	editorResources = std::make_unique<FEditor::Core::EditorResources>(editorAssetsPath);
	...
	ServiceLocator::Provide<FWindow::Inputs::InputManager>(*inputManager);
	ServiceLocator::Provide<FWindow::Window>(*window);
...
void FEditor::Core::Context::ResetProjectSettings()
{
...
bool FEditor::Core::Context::IsProjectSettingsIntegrityVerified()
{
	return
...
void FEditor::Core::Context::ApplyProjectSettings()
{
