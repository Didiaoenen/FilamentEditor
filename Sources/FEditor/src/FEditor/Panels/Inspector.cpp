...
#include <FEditorUI/Widgets/Drags/DragMultipleFloats.h>
...
FEditor::Panels::Inspector::Inspector
(
...
{
	m_inspectorHeader = &CreateWidget<FEditorUI::Widgets::Layout::Group>();
	m_inspectorHeader->enabled = false;
	m_actorInfo = &CreateWidget<FEditorUI::Widgets::Layout::Group>();

	auto& headerColumns = m_inspectorHeader->CreateWidget<FEditorUI::Widgets::Layout::Columns<2>>();
	...
	{
		auto& componentSelectorWidget = m_inspectorHeader->CreateWidget<FEditorUI::Widgets::Selection::ComboBox>(0);
		...
		auto& addComponentButton = m_inspectorHeader->CreateWidget<FEditorUI::Widgets::Buttons::Button>("Add Component", OvMaths::FVector2{ 100.f, 0 });
		addComponentButton.idleBackgroundColor = FEditorUI::Types::Color{ 0.7f, 0.5f, 0.f };
		addComponentButton.textColor = FEditorUI::Types::Color::White;
		...
		componentSelectorWidget.ValueChangedEvent += [this, &addComponentButton](int p_value)
		{
			auto defineButtonsStates = [&addComponentButton](bool p_componentExists)
			{
				...
				addComponentButton.idleBackgroundColor = !p_componentExists ? FEditorUI::Types::Color{ 0.7f, 0.5f, 0.f } : FEditorUI::Types::Color{ 0.1f, 0.1f, 0.1f };
			};
	...
	{
		m_scriptSelectorWidget = &m_inspectorHeader->CreateWidget<FEditorUI::Widgets::InputFields::InputText>("");
        m_scriptSelectorWidget->lineBreak = false;
		auto& ddTarget = m_scriptSelectorWidget->AddPlugin<FEditorUI::Plugins::DDTarget<std::pair<std::string, Layout::Group*>>>("File");
		
		auto& addScriptButton = m_inspectorHeader->CreateWidget<FEditorUI::Widgets::Buttons::Button>("Add Script", OvMaths::FVector2{ 100.f, 0 });
		addScriptButton.idleBackgroundColor = FEditorUI::Types::Color{ 0.7f, 0.5f, 0.f };
		addScriptButton.textColor = FEditorUI::Types::Color::White;
        ...
        const auto updateAddScriptButton = [&addScriptButton, this](const std::string& p_script)
        {
            ...
            addScriptButton.idleBackgroundColor = isScriptValid ? FEditorUI::Types::Color{ 0.7f, 0.5f, 0.f } : FEditorUI::Types::Color{ 0.1f, 0.1f, 0.1f };
        };
	...
	m_inspectorHeader->CreateWidget<FEditorUI::Widgets::Visual::Separator>();
...
void FEditor::Panels::Inspector::FocusActor(OvCore::ECS::Actor& p_target)
{
	if (m_targetActor)
		UnFocus();
...
void FEditor::Panels::Inspector::UnFocus()
{
	if (m_targetActor)
	{
...
void FEditor::Panels::Inspector::SoftUnFocus()
{
    if (m_targetActor)
    {
...
OvCore::ECS::Actor * FEditor::Panels::Inspector::GetTargetActor() const
{
	return m_targetActor;
}

void FEditor::Panels::Inspector::CreateActorInspector(OvCore::ECS::Actor& p_target)
{
...
void FEditor::Panels::Inspector::DrawComponent(OvCore::ECS::Components::AComponent& p_component)
{
	if (auto inspectorItem = dynamic_cast<OvCore::API::IInspectorItem*>(&p_component); inspectorItem)
	{
		auto& header = m_actorInfo->CreateWidget<FEditorUI::Widgets::Layout::GroupCollapsable>(p_component.GetName());
		...
		auto& columns = header.CreateWidget<FEditorUI::Widgets::Layout::Columns<2>>();
		columns.widths[0] = 200;
		inspectorItem->OnInspector(columns);
	}
}

void FEditor::Panels::Inspector::DrawBehaviour(OvCore::ECS::Components::Behaviour & p_behaviour)
{
	if (auto inspectorItem = dynamic_cast<OvCore::API::IInspectorItem*>(&p_behaviour); inspectorItem)
	{
		auto& header = m_actorInfo->CreateWidget<FEditorUI::Widgets::Layout::GroupCollapsable>(p_behaviour.name);
...
void FEditor::Panels::Inspector::Refresh()
{
	if (m_targetActor)
	{
		m_actorInfo->RemoveAllWidgets();
