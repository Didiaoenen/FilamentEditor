...
FEditor::Panels::Profiler::Profiler
(
	...
	m_frequency(p_frequency)
{
	...
	m_separator = &CreateWidget<FEditorUI::Widgets::Visual::Separator>();
...
void FEditor::Panels::Profiler::Update(float p_deltaTime)
{
	m_timer += p_deltaTime;
...
void FEditor::Panels::Profiler::Enable(bool p_value, bool p_disableLog)
{
	if (p_value)
	{
		if (!p_disableLog)
...
FEditorUI::Types::Color FEditor::Panels::Profiler::CalculateActionColor(double p_percentage) const
{
...
std::string FEditor::Panels::Profiler::GenerateActionString(OvAnalytics::Profiling::ProfilerReport::Action & p_action)
{
	std::string result;
