...
#include <FEditorUI/Widgets/Sliders/SliderFloat.h>
...
FEditor::Panels::MenuBar::MenuBar()
{
	CreateFileMenu();
	CreateBuildMenu();
	CreateWindowMenu();
...
void FEditor::Panels::MenuBar::HandleShortcuts(float p_deltaTime)
{
	auto& inputManager = *EDITOR_CONTEXT(inputManager);

	if (inputManager.GetKeyState(FWindow::Inputs::EKey::KEY_LEFT_CONTROL) == FWindow::Inputs::EKeyState::KEY_DOWN)
	{
		if (inputManager.IsKeyPressed(FWindow::Inputs::EKey::KEY_N))
			EDITOR_EXEC(LoadEmptyScene());

		if (inputManager.IsKeyPressed(FWindow::Inputs::EKey::KEY_S))
		{
			if (inputManager.GetKeyState(FWindow::Inputs::EKey::KEY_LEFT_SHIFT) == FWindow::Inputs::EKeyState::KEY_UP)
...
void FEditor::Panels::MenuBar::CreateFileMenu()
{
...
void FEditor::Panels::MenuBar::CreateBuildMenu()
{
...
void FEditor::Panels::MenuBar::CreateWindowMenu()
{
...
void FEditor::Panels::MenuBar::CreateSettingsMenu()
{
	...
	cameraSpeedMenu.CreateWidget<FEditorUI::Widgets::Sliders::SliderInt>(1, 50, 15, FEditorUI::Widgets::Sliders::ESliderOrientation::HORIZONTAL, "Scene View").ValueChangedEvent += EDITOR_BIND(SetSceneViewCameraSpeed, std::placeholders::_1);
	cameraSpeedMenu.CreateWidget<FEditorUI::Widgets::Sliders::SliderInt>(1, 50, 15, FEditorUI::Widgets::Sliders::ESliderOrientation::HORIZONTAL, "Asset View").ValueChangedEvent += EDITOR_BIND(SetAssetViewCameraSpeed, std::placeholders::_1);
	...
	auto& sceneViewBackgroundPicker = sceneViewBackground.CreateWidget<Selection::ColorEdit>(false, FEditorUI::Types::Color{ 0.098f, 0.098f, 0.098f });
    ...
    auto& sceneViewGridPicker = sceneViewGrid.CreateWidget<Selection::ColorEdit>(false, FEditorUI::Types::Color(0.176f, 0.176f, 0.176f));
	...
	sceneViewGrid.CreateWidget<MenuItem>("Reset").ClickedEvent += [this, &sceneViewGridPicker]
	{
		...
		sceneViewGridPicker.color = FEditorUI::Types::Color(0.176f, 0.176f, 0.176f);
	};

	auto& assetViewBackground = viewColors.CreateWidget<MenuList>("Asset View Background");
	auto& assetViewBackgroundPicker = assetViewBackground.CreateWidget<Selection::ColorEdit>(false, FEditorUI::Types::Color{ 0.098f, 0.098f, 0.098f });
	...
	auto& assetViewGridPicker = assetViewGrid.CreateWidget<Selection::ColorEdit>(false, FEditorUI::Types::Color(0.176f, 0.176f, 0.176f));
	...
	assetViewGrid.CreateWidget<MenuItem>("Reset").ClickedEvent += [this, &assetViewGridPicker]
	{
		...
		assetViewGridPicker.color = FEditorUI::Types::Color(0.176f, 0.176f, 0.176f);
	};

	auto& sceneViewBillboardScaleMenu = settingsMenu.CreateWidget<MenuList>("3D Icons Scales");
	auto& lightBillboardScaleSlider = sceneViewBillboardScaleMenu.CreateWidget<Sliders::SliderInt>(0, 100, static_cast<int>(Settings::EditorSettings::LightBillboardScale * 100.0f), FEditorUI::Widgets::Sliders::ESliderOrientation::HORIZONTAL, "Lights");
...
void FEditor::Panels::MenuBar::CreateLayoutMenu() 
{
	auto& layoutMenu = CreateWidget<MenuList>("Layout");
	layoutMenu.CreateWidget<MenuItem>("Reset").ClickedEvent += EDITOR_BIND(ResetLayout);
}

void FEditor::Panels::MenuBar::CreateHelpMenu()
{
    auto& helpMenu = CreateWidget<MenuList>("Help");
    helpMenu.CreateWidget<MenuItem>("GitHub").ClickedEvent += [] {FTools::Utils::SystemCalls::OpenURL("https://github.com/adriengivry/FilamentEditor"); };
    helpMenu.CreateWidget<MenuItem>("Tutorials").ClickedEvent += [] {FTools::Utils::SystemCalls::OpenURL("https://github.com/adriengivry/FilamentEditor/wiki/Tutorials"); };
    helpMenu.CreateWidget<MenuItem>("Scripting API").ClickedEvent += [] {FTools::Utils::SystemCalls::OpenURL("https://github.com/adriengivry/FilamentEditor/wiki/Scripting-API"); };
    helpMenu.CreateWidget<Visual::Separator>();
    helpMenu.CreateWidget<MenuItem>("Bug Report").ClickedEvent += [] {FTools::Utils::SystemCalls::OpenURL("https://github.com/adriengivry/FilamentEditor/issues/new?assignees=&labels=Bug&template=bug_report.md&title="); };
    helpMenu.CreateWidget<MenuItem>("Feature Request").ClickedEvent += [] {FTools::Utils::SystemCalls::OpenURL("https://github.com/adriengivry/FilamentEditor/issues/new?assignees=&labels=Feature&template=feature_request.md&title="); };
    helpMenu.CreateWidget<Visual::Separator>();
    helpMenu.CreateWidget<Texts::Text>("Version: 1.3.0");
}

void FEditor::Panels::MenuBar::RegisterPanel(const std::string& p_name, FEditorUI::Panels::PanelWindow& p_panel)
{
	auto& menuItem = m_windowMenu->CreateWidget<MenuItem>(p_name, "", true, true);
	menuItem.ValueChangedEvent += std::bind(&FEditorUI::Panels::PanelWindow::SetOpened, &p_panel, std::placeholders::_1);

	m_panels.emplace(p_name, std::make_pair(std::ref(p_panel), std::ref(menuItem)));
}

void FEditor::Panels::MenuBar::UpdateToggleableItems()
{
	for (auto&[name, panel] : m_panels)
		panel.second.get().checked = panel.first.get().IsOpened();
}

void FEditor::Panels::MenuBar::OpenEveryWindows(bool p_state)
{
	for (auto&[name, panel] : m_panels)
