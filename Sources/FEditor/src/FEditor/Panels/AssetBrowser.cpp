...
#define FILENAMES_CHARS FEditor::Panels::AssetBrowser::__FILENAMES_CHARS
...
class TexturePreview : public FEditorUI::Plugins::IPlugin
{
public:
	...
	FEditorUI::Widgets::Visual::Image image;
};

class BrowserItemContextualMenu : public FEditorUI::Plugins::ContextualMenu
{
public:
	...
	virtual void CreateList()
	{
		if (!m_protected)
		{
			auto& deleteAction = CreateWidget<FEditorUI::Widgets::Menu::MenuItem>("Delete");
			...
			{
				nameEditor.content = FTools::Utils::PathParser::GetElementName(filePath);
			...
			{
				if (!std::filesystem::is_directory(filePath))
					p_newName += '.' + FTools::Utils::PathParser::GetExtension(filePath);
				...
				std::string containingFolderPath = FTools::Utils::PathParser::GetContainingFolder(filePath);
...
public:
	bool m_protected;
	std::string filePath;
	FTools::Eventing::Event<std::string> DestroyedEvent;
	FTools::Eventing::Event<std::string, std::string> RenamedEvent;
};
...
public:
	...
	{
		auto& showInExplorer = CreateWidget<FEditorUI::Widgets::Menu::MenuItem>("Show in explorer");
		showInExplorer.ClickedEvent += [this]
		{
			FTools::Utils::SystemCalls::ShowInExplorer(filePath);
		};

		if (!m_protected)
		{
			auto& importAssetHere = CreateWidget<FEditorUI::Widgets::Menu::MenuItem>("Import Here...");
			importAssetHere.ClickedEvent += [this]
			{
				if (EDITOR_EXEC(ImportAssetAtLocation(filePath)))
				{
					FEditorUI::Widgets::Layout::TreeNode* pluginOwner = reinterpret_cast<FEditorUI::Widgets::Layout::TreeNode*>(userData);
					pluginOwner->Close();
					EDITOR_EXEC(DelayAction(std::bind(&FEditorUI::Widgets::Layout::TreeNode::Open, pluginOwner)));
				}
			};

			auto& createMenu = CreateWidget<FEditorUI::Widgets::Menu::MenuList>("Create..");

			auto& createFolderMenu = createMenu.CreateWidget<FEditorUI::Widgets::Menu::MenuList>("Folder");
			auto& createSceneMenu = createMenu.CreateWidget<FEditorUI::Widgets::Menu::MenuList>("Scene");
			auto& createShaderMenu = createMenu.CreateWidget<FEditorUI::Widgets::Menu::MenuList>("Shader");
			auto& createMaterialMenu = createMenu.CreateWidget<FEditorUI::Widgets::Menu::MenuList>("Material");

			auto& createStandardShaderMenu = createShaderMenu.CreateWidget<FEditorUI::Widgets::Menu::MenuList>("Standard template");
			auto& createStandardPBRShaderMenu = createShaderMenu.CreateWidget<FEditorUI::Widgets::Menu::MenuList>("Standard PBR template");
			auto& createUnlitShaderMenu = createShaderMenu.CreateWidget<FEditorUI::Widgets::Menu::MenuList>("Unlit template");
			auto& createLambertShaderMenu = createShaderMenu.CreateWidget<FEditorUI::Widgets::Menu::MenuList>("Lambert template");

			auto& createEmptyMaterialMenu = createMaterialMenu.CreateWidget<FEditorUI::Widgets::Menu::MenuList>("Empty");
			auto& createStandardMaterialMenu = createMaterialMenu.CreateWidget<FEditorUI::Widgets::Menu::MenuList>("Standard");
			auto& createStandardPBRMaterialMenu = createMaterialMenu.CreateWidget<FEditorUI::Widgets::Menu::MenuList>("Standard PBR");
			auto& createUnlitMaterialMenu = createMaterialMenu.CreateWidget<FEditorUI::Widgets::Menu::MenuList>("Unlit");
			auto& createLambertMaterialMenu = createMaterialMenu.CreateWidget<FEditorUI::Widgets::Menu::MenuList>("Lambert");

			auto& createFolder = createFolderMenu.CreateWidget<FEditorUI::Widgets::InputFields::InputText>("");
			auto& createScene = createSceneMenu.CreateWidget<FEditorUI::Widgets::InputFields::InputText>("");

			auto& createEmptyMaterial = createEmptyMaterialMenu.CreateWidget<FEditorUI::Widgets::InputFields::InputText>("");
			auto& createStandardMaterial = createStandardMaterialMenu.CreateWidget<FEditorUI::Widgets::InputFields::InputText>("");
			auto& createStandardPBRMaterial = createStandardPBRMaterialMenu.CreateWidget<FEditorUI::Widgets::InputFields::InputText>("");
			auto& createUnlitMaterial = createUnlitMaterialMenu.CreateWidget<FEditorUI::Widgets::InputFields::InputText>("");
			auto& createLambertMaterial = createLambertMaterialMenu.CreateWidget<FEditorUI::Widgets::InputFields::InputText>("");

			auto& createStandardShader = createStandardShaderMenu.CreateWidget<FEditorUI::Widgets::InputFields::InputText>("");
			auto& createStandardPBRShader = createStandardPBRShaderMenu.CreateWidget<FEditorUI::Widgets::InputFields::InputText>("");
			auto& createUnlitShader = createUnlitShaderMenu.CreateWidget<FEditorUI::Widgets::InputFields::InputText>("");
			auto& createLambertShader = createLambertShaderMenu.CreateWidget<FEditorUI::Widgets::InputFields::InputText>("");
			...
			createEmptyMaterial.EnterPressedEvent += [this](std::string materialName)
			{
				...
				{
					auto& materialEditor = EDITOR_PANEL(FEditor::Panels::MaterialEditor, "Material Editor");
...
public:
	FTools::Eventing::Event<std::string> ItemAddedEvent;
};
...
public:
	...
	virtual void CreateList() override
	{
		...
		auto& newScriptMenu = CreateWidget<FEditorUI::Widgets::Menu::MenuList>("New script...");
		auto& nameEditor = newScriptMenu.CreateWidget<FEditorUI::Widgets::InputFields::InputText>("");

		newScriptMenu.ClickedEvent += [this, &nameEditor]
		{
			nameEditor.content = FTools::Utils::PathParser::GetElementName("");
		};
...
public:
	...
	virtual void CreateList() override
	{
		auto& editAction = CreateWidget<FEditorUI::Widgets::Menu::MenuItem>("Open");

		editAction.ClickedEvent += [this]
		{
			FTools::Utils::SystemCalls::OpenFile(filePath);
		};

		if (!m_protected)
		{
			auto& duplicateAction = CreateWidget<FEditorUI::Widgets::Menu::MenuItem>("Duplicate");
			...
			duplicateAction.ClickedEvent += [this]
			{
				...
				std::string extension = "." + FTools::Utils::PathParser::GetExtension(filePath);
                ...
                const auto newNameWithoutExtension = FTools::Utils::String::GenerateUnique(filePathWithoutExtension, filenameAvailable);
        ...
        auto& editMetadata = CreateWidget<FEditorUI::Widgets::Menu::MenuItem>("Properties");

        editMetadata.ClickedEvent += [this]
        {
            auto& panel = EDITOR_PANEL(FEditor::Panels::AssetProperties, "Asset Properties");
...
public:
	FTools::Eventing::Event<std::string> DuplicateEvent;
};
...
public:
	...
	virtual void CreateList() override
	{
		auto& previewAction = CreateWidget<FEditorUI::Widgets::Menu::MenuItem>("Preview");
		...
		previewAction.ClickedEvent += [this]
		{
			...
			auto& assetView = EDITOR_PANEL(FEditor::Panels::AssetView, "Asset View");
...
public:
	...
	virtual void CreateList() override
	{
		...
		auto& compileAction = CreateWidget<FEditorUI::Widgets::Menu::MenuItem>("Compile");
...
public:
	...
	virtual void CreateList() override
	{
		auto& reloadAction = CreateWidget<FEditorUI::Widgets::Menu::MenuItem>("Reload");
		...
		if (!m_protected)
		{
			auto& generateMaterialsMenu = CreateWidget<FEditorUI::Widgets::Menu::MenuList>("Generate materials...");

			generateMaterialsMenu.CreateWidget<FEditorUI::Widgets::Menu::MenuItem>("Standard").ClickedEvent += [this]
			{
				auto& modelManager = OVSERVICE(OvCore::ResourceManagement::ModelManager);
				std::string resourcePath = EDITOR_EXEC(GetResourcePath(filePath, m_protected));
				if (auto model = modelManager.GetResource(resourcePath))
				{
					for (const std::string& materialName : model->GetMaterialNames())
					{
						...
						do
						{
							finalPath = FTools::Utils::PathParser::GetContainingFolder(filePath) + (!fails ? materialName : materialName + " (" + std::to_string(fails) + ')') + ".ovmat";
			...
			generateMaterialsMenu.CreateWidget<FEditorUI::Widgets::Menu::MenuItem>("StandardPBR").ClickedEvent += [this]
			{
				auto& modelManager = OVSERVICE(OvCore::ResourceManagement::ModelManager);
				std::string resourcePath = EDITOR_EXEC(GetResourcePath(filePath, m_protected));
				if (auto model = modelManager.GetResource(resourcePath))
			...
			generateMaterialsMenu.CreateWidget<FEditorUI::Widgets::Menu::MenuItem>("Unlit").ClickedEvent += [this]
			{
				auto& modelManager = OVSERVICE(OvCore::ResourceManagement::ModelManager);
				std::string resourcePath = EDITOR_EXEC(GetResourcePath(filePath, m_protected));
				if (auto model = modelManager.GetResource(resourcePath))
			...
			generateMaterialsMenu.CreateWidget<FEditorUI::Widgets::Menu::MenuItem>("Lambert").ClickedEvent += [this]
			{
				auto& modelManager = OVSERVICE(OvCore::ResourceManagement::ModelManager);
				std::string resourcePath = EDITOR_EXEC(GetResourcePath(filePath, m_protected));
				if (auto model = modelManager.GetResource(resourcePath))
	...
	virtual void CreateList() override
	{
		...
		reloadAction.ClickedEvent += [this]
		{
			...
			if (textureManager.IsResourceRegistered(resourcePath))
			{
				...
				EDITOR_PANEL(FEditor::Panels::MaterialEditor, "Material Editor").Refresh();
			}
		};
...
public:
	...
	virtual void CreateList() override
	{
		auto& editAction = CreateWidget<FEditorUI::Widgets::Menu::MenuItem>("Edit");
...
public:
	...
	virtual void CreateList() override
	{
		...
		auto& reload = CreateWidget<FEditorUI::Widgets::Menu::MenuItem>("Reload");
...
FEditor::Panels::AssetBrowser::AssetBrowser
(
	...
	m_projectScriptFolder(p_projectScriptFolder)
{
	if (!std::filesystem::exists(m_projectAssetFolder))
	{
		...
		FWindow::Dialogs::MessageBox message
		(
			"Assets folder not found",
			"The \"Assets/\" folders hasn't been found in your project directory.\nIt has been automatically generated",
			FWindow::Dialogs::MessageBox::EMessageType::WARNING,
			FWindow::Dialogs::MessageBox::EButtonLayout::OK
		);
	}
...
void FEditor::Panels::AssetBrowser::Fill()
{
	m_assetList->CreateWidget<FEditorUI::Widgets::Visual::Separator>();
...
void FEditor::Panels::AssetBrowser::Clear()
{
	m_assetList->RemoveAllWidgets();
}

void FEditor::Panels::AssetBrowser::Refresh()
{
	Clear();
	Fill();
}

void FEditor::Panels::AssetBrowser::ParseFolder(Layout::TreeNode& p_root, const std::filesystem::directory_entry& p_directory, bool p_isEngineItem, bool p_scriptFolder)
{
...
void FEditor::Panels::AssetBrowser::ConsiderItem(FEditorUI::Widgets::Layout::TreeNode* p_root, const std::filesystem::directory_entry& p_entry, bool p_isEngineItem, bool p_autoOpen, bool p_scriptFolder)
{
	bool isDirectory = p_entry.is_directory();
	std::string itemname = FTools::Utils::PathParser::GetElementName(p_entry.path().string());
	...
	FTools::Utils::PathParser::EFileType fileType = FTools::Utils::PathParser::GetFileType(itemname);

	// Unknown file, so we skip it
	if (fileType == FTools::Utils::PathParser::EFileType::UNKNOWN && !isDirectory)
	{
		return;
	}
	...
	if (isDirectory)
	{
		...
		auto& ddSource = treeNode.AddPlugin<FEditorUI::Plugins::DDSource<std::pair<std::string, Layout::Group*>>>("Folder", resourceFormatPath, std::make_pair(resourceFormatPath, &itemGroup));
		...
		contextMenu.ItemAddedEvent += [this, &treeNode, path, p_isEngineItem, p_scriptFolder] (std::string p_string)
		{
			...
			ParseFolder(treeNode, std::filesystem::directory_entry(FTools::Utils::PathParser::GetContainingFolder(p_string)), p_isEngineItem, p_scriptFolder);
		};
		...
		if (!p_scriptFolder)
		{
			...
			{
				treeNode.AddPlugin<FEditorUI::Plugins::DDTarget<std::pair<std::string, Layout::Group*>>>("Folder").DataReceivedEvent += [this, &treeNode, path, p_isEngineItem](std::pair<std::string, Layout::Group*> p_data)
				{
					if (!p_data.first.empty())
					{
						std::string folderReceivedPath = EDITOR_EXEC(GetRealPath(p_data.first));

						std::string folderName = FTools::Utils::PathParser::GetElementName(folderReceivedPath) + '\\';
				...
				treeNode.AddPlugin<FEditorUI::Plugins::DDTarget<std::pair<std::string, Layout::Group*>>>("File").DataReceivedEvent += [this, &treeNode, path, p_isEngineItem](std::pair<std::string, Layout::Group*> p_data)
				{
					if (!p_data.first.empty())
					{
						std::string fileReceivedPath = EDITOR_EXEC(GetRealPath(p_data.first));

						std::string fileName = FTools::Utils::PathParser::GetElementName(fileReceivedPath);
			...
			contextMenu.RenamedEvent += [this, &treeNode, path, &ddSource, p_isEngineItem](std::string p_prev, std::string p_newPath)
			{
				...
				if (!std::filesystem::exists(p_newPath)) // Do not rename a folder if it already exists
				{
					...
					std::string elementName = FTools::Utils::PathParser::GetElementName(p_newPath);
					std::string data = FTools::Utils::PathParser::GetContainingFolder(ddSource.data.first) + elementName + "\\";
			...
			{
				treeNode.RemoveAllWidgets();
				ParseFolder(treeNode, std::filesystem::directory_entry(FTools::Utils::PathParser::GetContainingFolder(p_path)), p_isEngineItem);
			};
		...
		{
			treeNode.RemoveAllWidgets();
			std::string updatedPath = FTools::Utils::PathParser::GetContainingFolder(path) + treeNode.name;
	...
	else
	{
		...
		switch (fileType)
		{
		case FTools::Utils::PathParser::EFileType::MODEL:		contextMenu = &clickableText.AddPlugin<ModelContextualMenu>(path, protectedItem);		break;
		case FTools::Utils::PathParser::EFileType::TEXTURE:	contextMenu = &clickableText.AddPlugin<TextureContextualMenu>(path, protectedItem); 	break;
		case FTools::Utils::PathParser::EFileType::SHADER:		contextMenu = &clickableText.AddPlugin<ShaderContextualMenu>(path, protectedItem);		break;
		case FTools::Utils::PathParser::EFileType::MATERIAL:	contextMenu = &clickableText.AddPlugin<MaterialContextualMenu>(path, protectedItem);	break;
		case FTools::Utils::PathParser::EFileType::SCENE:		contextMenu = &clickableText.AddPlugin<SceneContextualMenu>(path, protectedItem);		break;
		...
		auto& ddSource = clickableText.AddPlugin<FEditorUI::Plugins::DDSource<std::pair<std::string, Layout::Group*>>>
		(
			"File",
			resourceFormatPath,
		...
		contextMenu->RenamedEvent += [&ddSource, &clickableText, p_scriptFolder](std::string p_prev, std::string p_newPath)
		{
			if (p_newPath != p_prev)
			{
				if (!std::filesystem::exists(p_newPath))
				{
					RenameAsset(p_prev, p_newPath);
					std::string elementName = FTools::Utils::PathParser::GetElementName(p_newPath);
					ddSource.data.first = FTools::Utils::PathParser::GetContainingFolder(ddSource.data.first) + elementName;
		...
		if (fileType == FTools::Utils::PathParser::EFileType::TEXTURE)
		{
			auto& texturePreview = clickableText.AddPlugin<TexturePreview>();
			texturePreview.SetPath(resourceFormatPath);
		}

		if (fileType == FTools::Utils::PathParser::EFileType::SCENE)
		{
