...
#include <FEditorUI/Widgets/Layout/NewLine.h>
...
FEditor::Panels::AssetProperties::AssetProperties
(
	...
	PanelWindow(p_title, p_opened, p_windowSettings)
{
    ...
    m_headerSeparator = &CreateWidget<FEditorUI::Widgets::Visual::Separator>();
    m_headerSeparator->enabled = false;

    CreateAssetSelector();

    m_settings = &CreateWidget<FEditorUI::Widgets::Layout::GroupCollapsable>("Settings");
	m_settingsColumns = &m_settings->CreateWidget<FEditorUI::Widgets::Layout::Columns<2>>();
	m_settingsColumns->widths[0] = 150;

    m_info = &CreateWidget<FEditorUI::Widgets::Layout::GroupCollapsable>("Info");
    m_infoColumns = &m_info->CreateWidget<FEditorUI::Widgets::Layout::Columns<2>>();
    m_infoColumns->widths[0] = 150;

    m_settings->enabled = m_info->enabled = false;
}

void FEditor::Panels::AssetProperties::SetTarget(const std::string& p_path)
{
...
void FEditor::Panels::AssetProperties::Refresh()
{
    m_metadata.reset(new FTools::Filesystem::IniFile(EDITOR_EXEC(GetRealPath(m_resource)) + ".meta"));
    ...
    switch (FTools::Utils::PathParser::GetFileType(m_resource))
    {
...
void FEditor::Panels::AssetProperties::Preview()
{
	auto& assetView = EDITOR_PANEL(FEditor::Panels::AssetView, "Asset View");

	const auto fileType = FTools::Utils::PathParser::GetFileType(m_resource);

	if (fileType == FTools::Utils::PathParser::EFileType::MODEL)
	{
	...
	else if (fileType == FTools::Utils::PathParser::EFileType::TEXTURE)
	{
...
void FEditor::Panels::AssetProperties::CreateHeaderButtons()
{
	m_applyButton = &CreateWidget<FEditorUI::Widgets::Buttons::Button>("Apply");
	...
	m_revertButton = &CreateWidget<FEditorUI::Widgets::Buttons::Button>("Revert");
	...
	m_previewButton = &CreateWidget<FEditorUI::Widgets::Buttons::Button>("Preview");
	...
	m_resetButton = &CreateWidget<FEditorUI::Widgets::Buttons::Button>("Reset to default");
    ...
    m_headerLineBreak = &CreateWidget<FEditorUI::Widgets::Layout::NewLine>();
    m_headerLineBreak->enabled = false;
}

void FEditor::Panels::AssetProperties::CreateAssetSelector()
{
...
void FEditor::Panels::AssetProperties::CreateSettings()
{
...
void FEditor::Panels::AssetProperties::CreateInfo()
{
    ...
    if (std::filesystem::exists(realPath))
    {
        ...
        m_infoColumns->CreateWidget<FEditorUI::Widgets::Texts::Text>(realPath);

        OvCore::Helpers::GUIDrawer::CreateTitle(*m_infoColumns, "Size");
        const auto [size, unit] = FTools::Utils::SizeConverter::ConvertToOptimalUnit(static_cast<float>(std::filesystem::file_size(realPath)), FTools::Utils::SizeConverter::ESizeUnit::BYTE);
        m_infoColumns->CreateWidget<FEditorUI::Widgets::Texts::Text>(std::to_string(size) + " " + FTools::Utils::SizeConverter::UnitToString(unit));

        OvCore::Helpers::GUIDrawer::CreateTitle(*m_infoColumns, "Metadata");
        m_infoColumns->CreateWidget<FEditorUI::Widgets::Texts::Text>(std::filesystem::exists(realPath + ".meta") ? "Yes" : "No");
    }
    else
    {
...
void FEditor::Panels::AssetProperties::CreateModelSettings()
{
...
void FEditor::Panels::AssetProperties::CreateTextureSettings()
{
	...
	auto& minFilter = m_settingsColumns->CreateWidget<FEditorUI::Widgets::Selection::ComboBox>(m_metadata->Get<int>("MIN_FILTER"));
	...
	auto& magFilter = m_settingsColumns->CreateWidget<FEditorUI::Widgets::Selection::ComboBox>(m_metadata->Get<int>("MAG_FILTER"));
...
void FEditor::Panels::AssetProperties::Apply()
{
	m_metadata->Rewrite();
