...
FTools::Eventing::Event<FWindow::Context::EDeviceError, std::string> FWindow::Context::Device::ErrorEvent;

FWindow::Context::Device::Device(const Settings::DeviceSettings& p_deviceSettings)
{
	BindErrorCallback();
...
std::pair<int16_t, int16_t> FWindow::Context::Device::GetMonitorSize() const
{
...
GLFWcursor * FWindow::Context::Device::GetCursorInstance(Cursor::ECursorShape p_cursorShape) const
{
	return m_cursors.at(p_cursorShape);
}

bool FWindow::Context::Device::HasVsync() const
{
	return m_vsync;
}

void FWindow::Context::Device::SetVsync(bool p_value)
{
	glfwSwapInterval(p_value ? 1 : 0);
	m_vsync = p_value;
}

void FWindow::Context::Device::PollEvents() const
{
	glfwPollEvents();
}

float FWindow::Context::Device::GetElapsedTime() const
{
	return static_cast<float>(glfwGetTime());
}

void FWindow::Context::Device::BindErrorCallback()
{
...
void FWindow::Context::Device::CreateCursors()
{
...
void FWindow::Context::Device::DestroyCursors()
{
