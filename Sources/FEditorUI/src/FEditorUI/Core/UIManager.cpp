...
FEditorUI::Core::UIManager::UIManager(GLFWwindow* p_glfwWindow, Styling::EStyle p_style, const std::string& p_glslVersion)
{
	ImGui::CreateContext();
...
FEditorUI::Core::UIManager::~UIManager()
{
...
void FEditorUI::Core::UIManager::ApplyStyle(Styling::EStyle p_style)
{
	ImGuiStyle* style = &ImGui::GetStyle();

	switch (p_style)
	{
	case FEditorUI::Styling::EStyle::IM_CLASSIC_STYLE:	ImGui::StyleColorsClassic();	break;
	case FEditorUI::Styling::EStyle::IM_DARK_STYLE:		ImGui::StyleColorsDark();		break;
	case FEditorUI::Styling::EStyle::IM_LIGHT_STYLE:		ImGui::StyleColorsLight();		break;
	}

	if (p_style == FEditorUI::Styling::EStyle::DUNE_DARK)
	{
    ...
    else if (p_style == FEditorUI::Styling::EStyle::ALTERNATIVE_DARK)
    {
        style->WindowPadding = ImVec2(15, 15);
...
bool FEditorUI::Core::UIManager::LoadFont(const std::string& p_id, const std::string & p_path, float p_fontSize)
{
...
bool FEditorUI::Core::UIManager::UnloadFont(const std::string & p_id)
{
	if (m_fonts.find(p_id) != m_fonts.end())
	{
		m_fonts.erase(p_id);
...
bool FEditorUI::Core::UIManager::UseFont(const std::string & p_id)
{
...
void FEditorUI::Core::UIManager::UseDefaultFont()
{
	ImGui::GetIO().FontDefault = nullptr;
}

void FEditorUI::Core::UIManager::EnableEditorLayoutSave(bool p_value)
{
	if (p_value)
...
bool FEditorUI::Core::UIManager::IsEditorLayoutSaveEnabled() const
{
	return ImGui::GetIO().IniFilename != nullptr;
}

void FEditorUI::Core::UIManager::SetEditorLayoutSaveFilename(const std::string & p_filename)
{
...
void FEditorUI::Core::UIManager::SetEditorLayoutAutosaveFrequency(float p_frequency)
{
	ImGui::GetIO().IniSavingRate = p_frequency;
}

float FEditorUI::Core::UIManager::GetEditorLayoutAutosaveFrequency(float p_frequeny)
{
	return ImGui::GetIO().IniSavingRate;
}

void FEditorUI::Core::UIManager::EnableDocking(bool p_value)
{
...
void FEditorUI::Core::UIManager::ResetLayout(const std::string& p_config) const
{
    ImGui::LoadIniSettingsFromDisk(p_config.c_str());
}

bool FEditorUI::Core::UIManager::IsDockingEnabled() const
{
	return m_dockingState;
}

void FEditorUI::Core::UIManager::SetCanvas(Modules::Canvas& p_canvas)
{
	RemoveCanvas();
...
void FEditorUI::Core::UIManager::RemoveCanvas()
{
	m_currentCanvas = nullptr;
}

void FEditorUI::Core::UIManager::Render()
{
	if (m_currentCanvas)
	{
...
void FEditorUI::Core::UIManager::PushCurrentFont()
{

}

void FEditorUI::Core::UIManager::PopCurrentFont()
{
